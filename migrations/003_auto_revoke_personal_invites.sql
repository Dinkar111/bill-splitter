-- Run this once in your project's SQL Editor. Safe to re-run.
-- Fixes: a personal invite (tied to one member) stayed "active" forever
-- after being redeemed, even though it could never be used again.
...
-- Also clean up any personal invites already sitting stale from before this fix.
...
where member_id is not null
  ...
  and exists (
    ...
    where gm.id = group_invites.member_id and gm.user_id is not null
  );
